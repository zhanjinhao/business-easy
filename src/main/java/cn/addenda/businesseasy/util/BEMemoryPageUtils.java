...
public class BEMemoryPageUtils {
    ...
    public static <T> PageInfo<T> pageQuery(List<T> info, Integer pageNum, Integer pageSize, String orderBy, boolean isUnderline) {
        if (isUnderline) {
            return pageQuery(info, pageNum, pageSize, orderBy, BEMemoryPageUtils::underlineToCamel);
        }
    ...
    public static <T> List<T> order(List<T> infos, String orderBy, UnaryOperator<String> fieldMapping) {
        ...
        if (NO_ORDER == orderRulesMap || CollectionUtils.isEmpty(infos) || infos.size() == 1) {
            return infos;
        }
        // 取出泛型的真实类型
        T t = infos.get(0);
        ...
        Comparator<T> comparator = (Comparator<T>) comparatorMap.computeIfAbsent(key, s -> {
            return createComparator(infos, orderRulesMap);
        });
    ...
    public static <T> List<T> order(List<T> infos, String orderBy, boolean isUnderline) {
        if (isUnderline) {
            return order(infos, orderBy, BEMemoryPageUtils::underlineToCamel);
        }
        return order(infos, orderBy, null);
    }

    private static <T> String comparatorKey(Map<String, SqlOrder> orderRulesMap, T t) {
        Assert.notNull(t, "please make sure the parameter 't' is not null");
    ...
    private static <T> Comparator<T> createComparator(List<T> infos, Map<String, SqlOrder> orderRulesMap) {
        ...
        return (o1, o2) -> {
            ...
            try {
                ...
                throw new BEUtilException("error occurred on createComparator()", e);
            }
            return 0;
        };
    }
    ...
    private static Map<String, SqlOrder> orderRulesMap(String orderBy, UnaryOperator<String> fieldMapping) {
        ...
        if (!StringUtils.hasText(orderBy) || orderRules.length == 0) {
            return NO_ORDER;
        }
    ...
    private static Object getFieldData(Class<?> clazz, String property, Object o) throws Exception {
        Field field = fieldMap.computeIfAbsent(fieldKey(clazz, property), s -> {
            ...
            try {
                ...
                throw new BEUtilException("找不到对应的字段，字段名：" + property, e);
            }
        });
        if (field == null) {
            return null;
    ...
    static {
        try {
            ...
            throw new BEUtilException("当前类加载器无法找到java.lang.Comparable#compareTo", e);
        }
    }
